...
class RelayNetworkGUI:
    def __init__(self, root):
        ...
        self.create_menu()  # Call this to create the menu
    ...
    def create_input_frame(self):
        ...
        # Antenna Config
        ttk.Label(input_frame, text="Antenna Config:").grid(row=3, column=0, sticky=tk.W)
        ...
        for i, (key, value) in enumerate(configs.items()):
            ttk.Radiobutton(input_frame, text=value, variable=self.antenna_var,
                            value=key).grid(row=i + 3, column=1, sticky=tk.W)

        # Antenna Ranges
        self.surface_range_var = tk.StringVar(value="1.5")
        self.relay_range_var = tk.StringVar(value="5.0")
        ttk.Label(input_frame, text="Surface Range (Mm):").grid(row=6, column=0, sticky=tk.W)
        self.surface_entry = ttk.Entry(input_frame, textvariable=self.surface_range_var, state='disabled')
        self.surface_entry.grid(row=6, column=1, sticky=(tk.W, tk.E))
        ttk.Label(input_frame, text="Relay Range (Mm):").grid(row=7, column=0, sticky=tk.W)
        self.relay_entry = ttk.Entry(input_frame, textvariable=self.relay_range_var, state='disabled')
        self.relay_entry.grid(row=7, column=1, sticky=(tk.W, tk.E))

        # Custom Network Parameters
        ttk.Label(input_frame, text="Custom Orbit Height (Mm):").grid(row=8, column=0, sticky=tk.W)
        ...
        self.orbit_height_entry.grid(row=8, column=1, sticky=(tk.W, tk.E))

        ttk.Label(input_frame, text="Custom Relay Count:").grid(row=9, column=0, sticky=tk.W)
        ...
        self.custom_relay_entry.grid(row=9, column=1, sticky=(tk.W, tk.E))

        # Calculate Button
        ttk.Button(input_frame, text="Calculate Network", command=self.calculate_network).grid(row=10, column=0,
                                                                                               columnspan=2, pady=10)

        # Add trace for both antenna config and network type
    ...
    def custom_setup_gui(self):
        ...
        # Now adjust the relay count iteratively
        while not relay_los_check(body.radius, custom_orbit, relay_range, relay_count):
            relay_count += 1  # Increment the relay count if LOS check fails
            print(f"Trying {relay_count} relays...")  # Inform user of the attempt

        print(f"Final number of relays: {relay_count}")  # Show the final relay count

        # Now that we have the correct relay count, we can compute the relay distance again
        relay_distance = calc_relay_distance(body.radius, custom_orbit, relay_count)

        # Inform user when LOS is clear
        messagebox.showinfo("Info", "Line of sight between relays is clear with the given setup.")
    ...
    def update_results(self, result: RelaySetup, body: CelestialBody):
        self.results_text.delete(1.0, tk.END)
        ...
        results = [
            ...
            f"Feasible orbital window: {result['h_low']:.2f} Mm-{result['h_high']:.2f} Mm",
